...
\usepackage[orientation=portrait,size=a1,
            scale=1.10        % font scale factor
           ]{beamerposter}
           
\geometry{
  hmargin=2.0cm, % little modification of margins
  vmargin=2.0cm, % little modification of margins
}
...
The \textbf{Maximisation step} consists in re-estimating the model's parameters according to the previously computed probabilities. This step broadly consists in aggregating the $\gamma$ and $\xi$ probabilities for all $(\textbf{f},\textbf{e})$ sentence pairs, as per described in \cite{anahita}.
