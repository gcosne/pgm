...
# Call: python classify.py path_to_train path_to_test -m [1,2,3] -n [number_clusters]
# several methods can be called at the same time (plots will then appear on the same figure)
# Call example: python classify.py path/to/sets/EMGaussian.data	 path/to/sets/EMGaussian.test -m 1 2 3
...
#def log_likelihood(data_points,mu_vector,sigmas,pi_vector,Q):
#	result = 0
#	for i in range(len(data_points)):
#		best_class = 0;
#		best_proba = 0;
#		# for each point, we discriminate the most likely class (hard-assignment)
#		for k in range(len(sigmas)):
#			if (normal_law(data_points[i],mu_vector[k],sigmas[k]) > best_proba):
#				best_class = k
#				best_proba = normal_law(data_points[i],mu_vector[k],sigmas[k])
#
#		result += math.log(sys.float_info.epsilon + normal_law(data_points[i],mu_vector[best_class],sigmas[best_class]))+pi_vector[best_class]
#	return result
...
def question2(tr_data,test_data,n_states):
	...
	# compute messages
	print "alpha messages ..."
	alphas = compute_all_alpha(test_data,sigmas,mus,A,pi)
	print "beta messages ..."
	betas = compute_all_beta(test_data,sigmas,mus,A)
	
	# compute probabilities
	# first dimension is the hidden state index
	...
	axarr[0].plot(xaxis,probas[0,0:100],'r')
	axarr[0].set_ylabel('p(q_t=0|u_0...u_t)')
	axarr[1].plot(xaxis,probas[1,0:100],'b')
	axarr[1].set_ylabel('p(q_t=1|u_0...u_t)')
	axarr[2].plot(xaxis,probas[2,0:100],'g')
	axarr[2].set_ylabel('p(q_t=2|u_0...u_t)')
	axarr[3].plot(xaxis,probas[3,0:100],'k')
	axarr[3].set_ylabel('p(q_t=3|u_0...u_t)')
...
def main():
	# plot setttings
	supported_questions = np.array([2,4,5,6])
